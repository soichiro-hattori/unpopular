...
class CPM(object):
    ...
    def set_poly_model(self, time_interval, poly_terms, poly_reg):
        ...
        self.v_matrix = np.vander(
            self.scaled_centered_time, N=poly_terms, increasing=True
        )
    ...
    def set_target(self, target_row, target_col):
        ...
        self.target_fluxes = self.im_fluxes[
            :, target_row, target_col
        ]  # target pixel lightcurve
        self.target_errors = self.im_errors[
            :, target_row, target_col
        ]  # target pixel errors
        ...
        self.target_pixel_mask = np.ma.masked_where(
            target_pixel == 0, target_pixel
        )  # mask to see target
    ...
    def set_exclusion(self, exclusion, method="closest"):
        ...
        if method == "cross":
            excluded_pixels[
                max(0, r - exclusion) : min(r + exclusion + 1, im_side_length), :
            ] = 1
            excluded_pixels[
                :, max(0, c - exclusion) : min(c + exclusion + 1, im_side_length)
            ] = 1
        ...
        self.excluded_pixels_mask = np.ma.masked_where(
            excluded_pixels == 0, excluded_pixels
        )  # excluded pixel is "valid" and therefore False
        self.is_exclusion_set = True

    def set_predictor_pixels(
        self, num_predictor_pixels, method="similar_brightness", seed=None
    ):
        if seed != None:
        ...
        im_side_length = self.im_fluxes.shape[1]
        ...
        if method == "random":
            chosen_idx = np.random.choice(
                possible_idx, size=num_predictor_pixels, replace=False
            )

        if method == "similar_brightness":
            possible_pixel_medians = self.flattened_pixel_medians[
                self.excluded_pixels_mask.mask.ravel()
            ]
        ...
        if method == "cosine_similarity":
            possible_rescaled_im_fluxes = self.flattened_rescaled_im_fluxes[
                :, self.excluded_pixels_mask.mask.ravel()
            ]
            cos_similarity = np.dot(
                possible_rescaled_im_fluxes.T, self.rescaled_target_fluxes
            ) / (
                np.linalg.norm(possible_rescaled_im_fluxes.T, axis=1)
                * np.linalg.norm(self.rescaled_target_fluxes)
            )
            chosen_idx = possible_idx[
                np.argsort(cos_similarity)[::-1][0 : self.num_predictor_pixels]
            ]

        self.predictor_pixels_locations = np.array(
            [[idx // im_side_length, idx % im_side_length] for idx in chosen_idx]
        )
        ...
        self.rescaled_predictor_pixels_fluxes = self.rescaled_im_fluxes[
            :, loc[0], loc[1]
        ]
        self.predictor_pixels_mask = np.ma.masked_where(
            predictor_pixels == 0, predictor_pixels
        )
    ...
    def set_target_exclusion_predictors(
        ...
        self.set_predictor_pixels(
            num_predictor_pixels, method=predictor_method, seed=seed
        )
